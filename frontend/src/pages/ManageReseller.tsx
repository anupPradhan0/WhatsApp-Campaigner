...
interface ResellersData { totalResellers: number; resellers: Reseller[]; }

/* ── page ── */
const ManageReseller = () => {
  const [data,    setData]    = useState<ResellersData | null>(null);
  ...
  const [createForm, setCreateForm] = useState({ companyName:'', email:'', password:'', number:'', role:'reseller', balance:'', image: null as File|null });
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const { data: r } = await api.get<{ success: boolean; message?: string; data: ResellersData }>('/api/dashboard/manage-reseller');
  ...
  const totalPages  = Math.max(1, Math.ceil((data?.totalResellers || 0) / itemsPerPage));
  const startIdx    = (currentPage - 1) * itemsPerPage;
  const current     = data?.resellers.slice(startIdx, startIdx + itemsPerPage) || [];

  const formatDate = (s: string) => { try { return format(new Date(s), 'dd MMM yyyy, hh:mm a'); } catch { return s; } };

  const openModal = (type: typeof modal, r?: Reseller) => {
    setError(''); setSuccess('');
    if (r) setSelected(r);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (r.success) { closeModal(); setCreateForm({ companyName:'', email:'', password:'', number:'', role:'reseller', balance:'', image:null }); fetchData(); toast('Reseller created successfully!'); }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (r.success) { closeModal(); fetchData(); toast('Reseller deleted.'); } else setError(r.message || 'Failed');
    } catch { setError('Network error.'); }
    finally { setActionLoading(false); }
  };
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
          <div>
            ...
            <p style={{ fontSize:13, color:D.textMuted, marginTop:4 }}>{data?.totalResellers ?? 0} total resellers</p>
          </div>
        ...
        <div style={{ background:D.surface, border:`1px solid ${D.border}`, borderRadius:10, padding:'10px 16px', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
          ...
          <span style={{ marginLeft:'auto', fontSize:12, color:D.textSubtle }}>
            {startIdx + 1}–{Math.min(startIdx + itemsPerPage, data?.totalResellers || 0)} of {data?.totalResellers || 0}
          </span>
        </div>
