...
import { X, Eye, Edit2, Calendar, ChevronLeft, ChevronRight, Download, Loader2, Check, AlertCircle } from "lucide-react";
...
const statusColor = (s: string): [string, string] => {
  const m: Record<string, [string, string]> = { pending: [D.amber, D.amberDim], delivered: [D.greenLight, D.greenDim], processing: [D.blue, D.blueDim], failed: [D.red, D.redDim] };
...
interface Campaign { campaignId: string; campaignName: string; status: string; statusMessage: string; message: string; createdBy: string; mobileNumberCount: number; createdAt: string; image: string; userData: { companyName: string; email: string; number: string; role: string; status: string; createdAt: string; }; }
...
export default function AllCampaigns() {
  ...
  const fetchData = useCallback(async () => {
    try { setLoading(true); const { data: r } = await api.get('/api/dashboard/all-campaigns'); if (r.success) setData(r.data); else setError(r.message || 'Failed'); } catch { setError('Network error.'); } finally { setLoading(false); }
  }, []);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      if (r.success) { toast.success('Status updated!'); setEditCampaign(null); fetchData(); } else toast.error(r.message || 'Failed');
    } catch { toast.error('Error updating status'); } finally { setUpdatingStatus(false); }
  };
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 400, flexDirection: 'column', gap: 12 }}>
      ...
      <p style={{ color: D.textMuted, fontSize: 13 }}>Loading campaigns…</p>
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {/* Header */}
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: D.text, margin: 0 }}>All Campaigns</h1>
          <p style={{ fontSize: 13, color: D.textMuted, marginTop: 4 }}>Latest 50 campaigns from all users</p>
        </div>
        ...
        <div className="hidden md:block" style={{ background: D.surface, border: `1px solid ${D.border}`, borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {paginated.length === 0 ? <tr><td colSpan={8} style={{ padding: '40px', textAlign: 'center', color: D.textSubtle, fontSize: 13 }}>No campaigns found.</td></tr>
                  : paginated.map((c, i) => (
                  <tr key={c.campaignId} className="row-h" style={{ borderBottom: `1px solid rgba(39,39,42,0.5)` }}>
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        {userRole === UserRole.ADMIN && <button onClick={() => { setEditCampaign(c); setUpdateStatus(c.status || 'pending'); setUpdateMessage(''); }} title="Edit Status" style={{ width: 30, height: 30, borderRadius: 7, background: D.blueDim, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><Edit2 size={12} style={{ color: D.blue }} /></button>}
          ...
          {paginated.length === 0 ? <div style={{ padding: 32, textAlign: 'center', background: D.surface, border: `1px solid ${D.border}`, borderRadius: 12 }}><p style={{ color: D.textSubtle, fontSize: 13 }}>No campaigns found.</p></div>
            ...
            <div key={c.campaignId} style={{ background: D.surface, border: `1px solid ${D.border}`, borderRadius: 10, padding: '12px 14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><StatusBadge s={c.status} /><span style={{ fontSize: 11, fontWeight: 600, color: D.blue, background: D.blueDim, padding: '2px 7px', borderRadius: 20 }}>{c.mobileNumberCount}</span></div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, paddingTop: 8, borderTop: `1px solid ${D.border}` }}>
                {userRole === UserRole.ADMIN && <button onClick={() => { setEditCampaign(c); setUpdateStatus(c.status || 'pending'); setUpdateMessage(''); }} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', background: D.blueDim, border: 'none', borderRadius: 6, cursor: 'pointer', color: D.blue, fontSize: 12, fontWeight: 600 }}><Edit2 size={12} /> Edit</button>}
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ background: D.surface2, border: `1px solid ${D.border}`, borderRadius: 10, padding: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  <div><p style={{ fontSize: 10, color: D.textSubtle, fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>Status</p><StatusBadge s={selected.status} /></div>
                </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: D.surface, border: `1px solid ${D.border}`, borderRadius: 14, width: '100%', maxWidth: 420 }}>
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ background: D.surface2, border: `1px solid ${D.border}`, borderRadius: 8, padding: 12 }}>
                ...
                <p style={{ fontSize: 12, color: D.textMuted, marginTop: 4 }}>Current: <StatusBadge s={editCampaign.status} /></p>
              </div>
              <div>
                <p style={{ fontSize: 12, color: D.textMuted, fontWeight: 600, marginBottom: 6 }}>New Status <span style={{ color: D.red }}>*</span></p>
                <select value={updateStatus} onChange={e => setUpdateStatus(e.target.value)} style={selStyle}>
              ...
              <div>
                ...
                <textarea value={updateMessage} onChange={e => setUpdateMessage(e.target.value)} rows={3} placeholder={`Current: ${editCampaign.statusMessage || 'None'}`} style={taStyle} />
              </div>
